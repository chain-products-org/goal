...
func TestRawCmd(t *testing.T) {
	r := cmd.Exec("bash", "-c", "/Users/home/tools/ord-0.16.0/ord --chain testnet --bitcoin-rpc-url http://10.10.10.207:18332 wallet --name test1 --server-url http://10.10.10.207:80 inscriptions")
	if len(r) > 2 {
...
var setting = ord.OrdSetting{Ord: ord.Setting{
	BinPath:       "/Users/home/tools/ord-0.16.0/ord",
	ChainNet:      "testnet",
...
func TestInscribe(t *testing.T) {
	client := ord.NewClient(setting)
	file := "/Users/home/tools/ord-0.16.0/test_ins.txt" // 本地文件
	r, err := client.Inscribe("test1", "your addr", 1, file, ord.Postage(546), ord.DryRun())
	if err != nil {
		t.Fatal(err)
	}
...
func TestInscribeBatch(t *testing.T) {
	client := ord.NewClient(setting)
	file := "/Users/home/tools/ord-0.16.0/test_batch_ins.txt"
